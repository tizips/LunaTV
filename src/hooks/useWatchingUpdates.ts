...
export function useWatchingUpdatesQuery(options?: {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<WatchingUpdate> => {
      ...
      const playRecords = await queryClient.ensureQueryData(
        playRecordsQueryOptions,
      );
      const playRecordsArray = Object.entries(playRecords)
        ...
        if (reminders) {
          ...
          const newReleases = Object.entries(reminders)
            .filter(([_key, reminder]) => {
              ...
              // 检查是否已经在播放记录中（避免重复）
              const isInPlayRecords =
                playRecordsArray &&
                playRecordsArray.some(
